...
zero_local_jobs_test()
{
    ...
    ICECC_TEST_SOCKET="$testdir"/socket-localice ICECC_TEST_REMOTEBUILD=1 ICECC_PREFERRED_HOST=remoteice1 ICECC_DEBUG=debug ICECC_LOGFILE="$testdir"/icecc.log $valgrind "${icecc}" $GXX -Wall -Werror -c testfunc.cpp -o "${testdir}/testfunc.o" 2>&1 >>"$testdir"/stderr.log
    result=$?
    echo "%%%%%%%%%%%%%%%%%%% $result"
